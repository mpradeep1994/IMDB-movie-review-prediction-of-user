...
"""
build a text classifier to determine whether a
dsdmovie review is expressing positive or negative sentiment. The data come from
...
def token_pair_features(tokens, feats, k=3):
    ...
    for out in range(len(window)):
        for i in range(3):
            j=i+1
            while(j<3):
                feats[prefix+ window[out][i]+"__"+window[out][j]]+=1
                j+=1
...
def mean_accuracy_per_setting(results):
    ...
    return sorted(mean_list, key = lambda x:x[0], reverse = True) 
